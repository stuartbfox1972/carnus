  ...
  return (
    <section>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h3 style={{ margin: 0 }}>
          {/* FIX: Use optional chaining and fallback to prevent 'replace' crash */}
          Results for: <span style={{ color: '#3b82f6' }}>{selectedTag?.replace('TAG#', '') || 'Loading...'}</span>
        </h3>
        <button
          onClick={onBack}
          ...
          >
            <div style={{ width: '100%', height: '200px', overflow: 'hidden', background: '#f1f5f9' }}>
              <img
                ...
                alt={img.ImageName || "Bird image"}
                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                // Simple error handling for broken thumbnails
                onError={(e) => { e.target.src = 'https://via.placeholder.com/200?text=No+Thumb'; }}
              />
            </div>
